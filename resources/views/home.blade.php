...
@section('content')
            ...
            <div style="background: #fff; border: 1px solid #ddd; border-radius: 8px; overflow: hidden; display: flex; flex-direction: column; transition: transform 0.2s; box-shadow: 0 2px 5px rgba(0,0,0,0.1);">
                ...
                <div style="height: 200px; overflow: hidden; background: #f0f0f0;">
                    <a href="{{ route('articles.show', $article->id) }}" title="Посмотреть оригинал">
                        <img src="{{ asset('images/' . ($article->preview_image ?? 'preview.jpg')) }}"
                             alt="{{ $article->name }}"
    ...
    <div style="text-align: center; margin-top: 40px;">
        <a href="{{ route('articles.index') }}" style="display: inline-block; padding: 12px 25px; background: #1a1a1a; color: #fff; text-decoration: none; border-radius: 5px; font-weight: bold;">
            Все новости
        </a>
    </div>
@endsection